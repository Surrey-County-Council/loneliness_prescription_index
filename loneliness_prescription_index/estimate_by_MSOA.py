...
geography_column = 'MSOA11CD'
# TODO source the correct shp file
boundary_file = path.join("data", "boundaries", "MSOA_2011", "Middle_Layer_Super_Output_Areas__December_2011__Boundaries_Full_Extent__BFE__EW_V3.shp")
# TODO use the correct output file
prescriptions_file = path.join("output", "presc_2020.csv")
output_folder = "output"

""" -------------------------------
    Interpolate at MSOA using IDW
------------------------------- """
...
# Load GP data
gp_df = pd.read_csv(prescriptions_file).rename(columns={"msoa11": geography_column})
...
f.savefig(path.join(output_folder, f"MSOA_loneliness_interpolation_k%d.png" % k))

f, _ = plot_attribute(maps, "LISA_score", cmap="Reds")
f.savefig(path.join(output_folder, f"MSOA_LISA_k%d.png" % k))
...
maps[[geography_column, 'loneliness', 'LISA_score', 'LISA_support']]\
    ...
    .to_csv(path.join(output_folder, f"MSOA_loneliness_stats_k%d.csv" % k), index=False)
...
maps[[geography_column, 'loneliness', 'ITEMS']]\
    .sort_values("loneliness", ascending=False)\
    .to_csv(path.join(output_folder, "MSOA_loneliness_agg.csv"), index=False)

# Plot results
f, ax1 = plot_attribute(maps, "loneliness", cmap="Blues")
f.savefig(path.join(output_folder, "MSOA_loneliness_agg.png"))
